...
const Button = ({ children, onClick, variant = 'primary', className = '', disabled = false, type = 'button' }) => {
  ...
  const variants = {
    ...
    warning: "bg-orange-50 text-orange-700 border border-orange-200 hover:bg-orange-100" // New variant for Handover
  };
  return (
...
function App() {
  ...
  // DATA STATES
  const [lostReports, setLostReports] = useState([]); // Stores broadcasted alerts
  const [myFoundItems, setMyFoundItems] = useState([]); // NEW: Stores items found by ME
  ...
  // 2. Fetch Broadcasted Lost Reports (Existing)
  useEffect(() => {
  ...
  // 3. NEW: Fetch "My Found Items" to check for Handover logic
  useEffect(() => {
    if (!user) return;
    // Query items found by THIS user only
  ...
  // 4. NEW: Helper to check if item is > 7 days old
  ...
  // 5. NEW: Handle Handover to Security
  ...
  // Existing: Handle Deleting a Broadcast Report
  ...
  // UPLOAD HANDLER (Updated to include default status)
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        await addDoc(collection(db, "items"), {
          ...
          email: user.email,
          phoneNumber: mobile,
          status: "With Student" // NEW: Default status tracking
        });
      ...
      console.error(error);
      showToast("AI Analysis failed. Try a clearer photo.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const Dashboard = () => {
    // Check for any overdue items that haven't been handed over yet
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500">
        ...
        {/* 🚨 NEW: URGENT NOTIFICATIONS (HANDOVER PROTOCOL) */}
                ...
                <Card key={report.id} className="p-5 hover:shadow-md transition-shadow">
                  ...
                  <div className="flex gap-2 border-t pt-3">
                    ...
                    {user.uid === report.userId ? (
                      ...
                      /* IF USER IS NOT OWNER -> SHOW CONTACT BUTTON */
                      <Button 
                        variant="secondary" 
                        className="w-full text-sm py-1.5"
                        onClick={() => alert(`Contact Owner: ${report.contactName}\nPhone: ${report.contactPhone}\nEmail: ${report.contactEmail}`)}
                      >
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      {toast && (
        <div className={`fixed top-4 right-4 z-50 px-4 py-3 rounded-lg shadow-lg flex items-center gap-2 animate-in slide-in-from-top-2 fade-in duration-300 ${toast.type === 'error' ? 'bg-red-600 text-white' : 'bg-slate-900 text-white'}`}>
